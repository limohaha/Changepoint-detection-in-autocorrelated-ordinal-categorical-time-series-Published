...
Abq <- read_csv("5AbqRainfall/data/Abq19310301_20201211.csv",
...
# Discretized by spectral envelope
tmp <- myfreq(X_hour_wide)
tmp[1:10, ]
#          [,1]        [,2]
# [1,] 368.1818 0.003304273
# [2,] 337.5000 0.003172700
# [3,] 405.0000 0.002911133
# [4,] 311.5385 0.002827733
# [5,] 450.0000 0.002597365
# [6,] 289.2857 0.002466251
# [7,] 176.0870 0.002368804
# [8,] 184.0909 0.002268755
# [9,] 168.7500 0.002205673
# [10,] 506.2500 0.002137145
...
# Probability
TestStatistic <- 1.045
load(paste("2CV/cvY/finaldataK3/TRDSEA_alpha0.3_B0.3_D-0.5_phi0.5_T10000CV.RData"))
Prob <- 1 - length(which(CPSim$val > TestStatistic))/dim(CPSim)[1]
Prob
# [1] 0.986925
...
# xtable(t(parMatrix2Y), digits = 4)
# xtable(matrix(mySE2Y, nrow=1), digits = 4)
...
# xtable(t(parMatrix2Z), digits = 4)
# xtable(matrix(mySE2Z, nrow=1), digits = 4)
...
#      xaxt="n", yaxt="n",
#      xlim=c(-100, Ts+100), ylim=c(1,3.3), xaxs="i")
# tmp <- c(1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000)
# px <- rep(0, length(tmp))
# for(i in 1:length(tmp)){
#   px[i] <- which(as.numeric(format(mydat$DATE, "%Y")) == tmp[i])[1]
...
# legend(1200, 3.4, horiz = T, legend=c("Before Changepoint", "After Changepoint"),
...
# # setEPS()
# # postscript("/Users/limo/Desktop/ClipVerify/PaperScriptsFinal/pic/ApplyFigure1_YZ.eps",
# #            width = 14, height = 7)
# par(cex.lab=1.2, cex.axis=1.2, mfrow=c(1,1))
# plot(x=1:Ts, y=mydat$Catg+1, type="l", 
#      col="darkgrey",
#      xlab="Day", ylab="Rainfall",
#      xaxt="n", yaxt="n", 
#      xlim=c(-30, Ts+30), ylim=c(1,3.2), xaxs="i")
# tmp <- c(1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000)
# px <- rep(0, length(tmp))
# for(i in 1:length(tmp)){
#   px[i] <- which(as.numeric(format(mydat$DATE, "%Y")) == tmp[i])[1]
...
# lines(1:(tauestY-1), 
#       EXLHaY[1:(tauestY-1)], 
#       type="l", lty="solid", col="black", lwd=2)
# lines(tauestY:Ts, 
#       EXLHaY[tauestY:Ts], 
#       type="l", lty="solid", col="black", lwd=2)
# lines(1:(tauestZ-1), 
#       EXLHaZ[1:(tauestZ-1)], 
#       type="l", lty="dashed", col="red", lwd=2.3)
# lines(tauestZ:Ts, 
#       EXLHaZ[tauestZ:Ts], 
#       type="l", lty="dashed", col="red", lwd=2.3)
# # legend(2*Ts/5-200, 3.3, horiz = T, legend=c(expression(italic(M)[I]),
# #                                       expression(tilde(italic(M))[epsilon])),
# #        col=c("black", "red"), lwd = c(2, 2.3),
# #        lty=c("solid", "dashed"), cex=1, bty="n")
# legend("top", horiz = T, 
#        legend=c(expression(paste("Fitted Mean with ", hat(tau)[I], sep="")), 
#                 expression(paste("Fitted Mean with ", hat(tau)[epsilon], sep=""))),
#        col=c("black", "red"), lwd = c(2, 2.3),
#        lty=c("solid", "dashed"), cex=1, bty="n")
# # dev.off()
